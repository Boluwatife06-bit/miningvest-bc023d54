...
import { ShieldCheck, Users, Wallet, BarChart3, CheckCircle, XCircle } from "lucide-react";
import { Navigate } from "react-router-dom";

type Tab = "deposits" | "investments" | "users";
    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="px-4 mt-4 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="bg-card border border-border rounded-xl p-3 text-center">
            <Wallet className="w-5 h-5 text-yellow-400 mx-auto" />
            <p className="text-lg font-bold text-foreground mt-1">{stats.pendingDeposits}</p>
            <p className="text-[10px] text-muted-foreground">Pending Dep.</p>
          </div>
          <div className="bg-card border border-border rounded-xl p-3 text-center">
        ...
        <div className="flex gap-1 bg-secondary rounded-xl p-1">
          {([["deposits", "Deposits"], ["investments", "Investments"], ["users", "Users"]] as const).map(([t, label]) => (
            <button
              key={t}
              onClick={() => setTab(t)}
